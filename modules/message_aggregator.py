...
class LastMessageAggregator(MessageAggregator):
    ...
    def aggregate(self, node_ids, messages, cor_destination=None):
...
class UniqueMessageAggregator(MessageAggregator):
    def __init__(self, device):
        super(UniqueMessageAggregator, self).__init__(device)
    ...
    def aggregate(self, node_ids, messages, cor_destination=None):
        ...
        for node_id in unique_node_ids:
            cor_des = cor_destination[node_id]
            # cor_des = np.flip(cor_des)
            unique_des, index = np.unique(cor_des, return_index=True)
            unique_messages_in = []
            unique_timestamps_in = []

            for i in range(index.shape[0]):
                unique_messages_in.append(messages[node_id][index[i]][0])
                unique_timestamps_in.append(messages[node_id][index[i]][1])
                # unique_destinations.append(self.cor_destination[n][index[i]])
            if index.shape[0] > 0:
                to_update_node_ids.append(node_id)
                unique_messages.append(torch.mean(torch.stack(unique_messages_in), dim=0))
                unique_timestamps.append(unique_timestamps_in[-1])
            # if len(messages[node_id]) > 0:
            #     n_messages += len(messages[node_id])
            #     to_update_node_ids.append(node_id)
            #     unique_messages.append(torch.mean(torch.stack([m[0] for m in messages[node_id][index]]), dim=0))
            #     unique_timestamps.append(messages[node_id][-1][1])
...
def get_message_aggregator(aggregator_type, device):
    ...
    elif aggregator_type == "unique":
        return UniqueMessageAggregator(device=device)
    else:
