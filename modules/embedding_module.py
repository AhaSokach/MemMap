...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
    ...
    source_nodes_torch = source_nodes# torch.from_numpy(source_nodes).long().to(self.device)
    # timestamps_torch = torch.unsqueeze(torch.from_numpy(timestamps).float().to(self.device), dim=1)

    # query node always has the start time -> time span == 0
    source_nodes_time_embedding = None#self.time_encoder(torch.zeros_like(
      # timestamps_torch)) # (600,1,172)
    ...
    if n_layers == 0:
      ...
      latent=False
      if latent:
        neighbors =  self.neighbor_finder.get_latent_neighbor(
      ...
      # edge_deltas_torch = torch.from_numpy(edge_deltas).float().to(self.device) # 600, 10
      ...
      edge_time_embeddings = None #self.time_encoder(edge_deltas_torch)
