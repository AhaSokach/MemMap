...
def get_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
    ...
    if dataset_name == "enron":
        #graph_df['Duplicates_Count'] = graph_df.duplicated(subset=['u', 'i','ts']).groupby(graph_df.columns[1:4].tolist()).transform('sum')
        graph_df = graph_df.drop_duplicates(subset=['u', 'i','ts'])
        #edge_features = graph_df['Duplicates_Count']
    else:
        graph_df = graph_df.drop_duplicates(subset=['u', 'i', 'ts'])
    ...
    full_data1 = Data(sources[observed_edges_mask], destinations[observed_edges_mask], timestamps[observed_edges_mask],
                      edge_idxs[observed_edges_mask], labels[observed_edges_mask])
